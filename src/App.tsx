...
const AppContent: React.FC = () => {
  ...
  const inspector = useElementInspector({
    content: state.wordingData?.content ?? null,
  });

  // Surface a site-info load failure as a native Designer notification.
  useEffect(() => {
    if (siteInfoError) {
      void webflow.notify({
        type: "Error",
        message: "Impossible de charger les informations du site Webflow",
      });
    }
  }, [siteInfoError]);
  ...
  const handleScan = useCallback(async () => {
    ...
    if (deployer.isMultiPage()) {
      ...
      try {
        ...
        void webflow.notify({
          type: "Info",
          message: `Scan terminé : ${totalElements} éléments trouvés`,
        });
      } catch (err) {
        actions.setError(
          "Erreur de scan: " +
        ...
        void webflow.notify({ type: "Error", message: "Erreur de scan" });
      }
    } else {
      // Single page mode
      ...
      try {
        ...
        void webflow.notify({
          type: "Info",
          message: `Scan terminé : ${elementsCount} éléments trouvés`,
        });
      } catch (err) {
        actions.setError(
          "Erreur de scan: " +
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      if (isSinglePage) {
        ...
        void webflow.notify({
          type: "Success",
          message: `Déploiement réussi : ${report.stats.applied} changements appliqués`,
        });
      } else {
        actions.setStep("scan-progress");
        actions.setScanProgress({
          currentPage: "Démarrage du déploiement...",
          completed: 0,
          total: 0,
          mode: "deploy",
        });

        const report = await deployer.deployToAllPages((progress) => {
      ...
      actions.setStep("input");
      void webflow.notify({ type: "Error", message: "Erreur de déploiement" });
    } finally {
      actions.setLoading(false);
    }
  ...
  // Site info failed to load — show a dedicated error screen instead of an
  // empty header (which would surface a misleading "site_id manquant" later).
  if (siteInfoError && !siteInfo) {
    return (
      <div className="app-container">
        <div className="app-inner">
          <div className="app-header">
            <div className="app-header-top">
              ...
              <p className="app-subtitle">Erreur de chargement</p>
            </div>
            <div className="app-header-bottom">
              {isLocalhost ? <span className="badge-local">LOCAL</span> : null}
              {inspectorButton}
          ...
          <div className="app-error-banner" role="alert">
            Impossible de récupérer les informations du site Webflow :{" "}
            {siteInfoError}. Vérifiez que l'extension est ouverte dans le
            Designer, puis rouvrez-la.
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div
      className={`app-container${inspector.active ? " inspector-active" : ""}`}
    >
      <div className="app-inner">
        <div className="app-header">
          <div className="app-header-top">
        ...
        {state.step === "preview" && state.previewData ? (
          <div key="step-preview" className="step-enter">
            <AuditSection
              content={state.wordingData?.content ?? {}}
      ...
      {inspector.active ? (
        <ElementInspector elementInfo={inspector.elementInfo} />
      ) : null}
    </div>
  );
};
