...
import { PreviewData, Change, PagePreview, SectionGroup } from "../types";
import {
  MAX_DISPLAYED_WARNINGS,
  FALLBACK_SECTION_NAME,
  MAX_PREVIEW_VALUE_LENGTH,
...
const groupChangesBySection = (changes: Change[]): SectionGroup[] => {
  ...
  return Array.from(groups.entries())
    ...
    .map(([sectionName, sectionChanges]) => ({
      sectionName,
...
const SectionGroupView: React.FC<{ sections: SectionGroup[] }> = ({
  sections,
}) => (
  <>
    {sections.map((section) => (
      <div key={section.sectionName} style={{ marginBottom: "16px" }}>
        <div
          style={{
            display: "flex",
            alignItems: "center",
          ...
          <div className="flex-between mb-4">
            ...
            {previewData.missingKeys && previewData.missingKeys.length > 0 ? (
              <span className="badge badge-warning">
                ...
                <span
                  className="info-tip"
                  data-tooltip="Clés présentes sur Webflow mais non renseignées dans le fichier importé"
                >
                  &#9432;
                </span>
              </span>
      ...
      {!isSinglePage && previewData.summary ? (
        <>
          ...
          >
            ...
            {previewData.summary.unusedKeys &&
            previewData.summary.unusedKeys.length > 0 ? (
              <span className="badge badge-warning">
                {previewData.summary.unusedKeys.length} clés non trouvées
                <span
                  className="info-tip"
                  data-tooltip="Clés présentes dans le fichier importé mais introuvables sur Webflow"
                >
                  &#9432;
                </span>
              </span>
          ...
          previewData.summary.unusedKeys.length > 0 ? (
            ...
            >
              ...
              {previewData.summary.unusedKeys
                .slice(0, MAX_DISPLAYED_WARNINGS)
                .map((key: string) => (
                  <div key={key} style={{ marginBottom: "4px" }}>
                    * {key}
                  </div>
                ))}
              {previewData.summary.unusedKeys.length >
              MAX_DISPLAYED_WARNINGS ? (
          ...
          >
            {previewData.pagesPreviews?.map((pagePreview: PagePreview) => (
              <details
                key={pagePreview.pageName}
                className="card"
              ...
              >
                  ...
                  >
                    ...
                    {pagePreview.stats.missing > 0 ? (
                      <span className="badge badge-warning">
                        ...
                        <span
                          className="info-tip"
                          data-tooltip="Clés présentes sur cette page mais non renseignées dans le fichier importé"
                        >
                          &#9432;
                        </span>
                      </span>
                ...
                >
                  <SectionGroupView
                    sections={groupChangesBySection(pagePreview.changes)}
                  />
                  ...
                  {pagePreview.missingKeys.length > 0 ? (
                    <div style={{ marginTop: "16px" }}>
                      ...
                      {pagePreview.missingKeys.map((key: string) => (
