...
export const ResultSection: React.FC<ResultSectionProps> = ({
  ...
  const isMultiPage =
    report.multiPageReports && report.multiPageReports.length > 0;
  ...
  const seoSuccess =
    report.seoChanges?.filter((s) => s.status === "success").length ?? 0;
  const seoErrors =
    report.seoChanges?.filter((s) => s.status === "error").length ?? 0;
  ...
  const animatedSkipped = useCountUp(report.stats.skipped);
  ...
  return (
    <div className="section">
      ...
      <div className="card mb-4">
        <div
          className="flex-between stat-row"
          style={{ marginBottom: "10px", animationDelay: "0ms" }}
        >
          <span style={{ fontSize: "12px", color: "var(--text-secondary)" }}>
            Appliqué
          </span>
          <span
            className="badge badge-success stat-value"
            style={{ margin: 0 }}
          >
            {animatedApplied}
          </span>
        ...
        {report.stats.failed > 0 ? (
          <div
            className="flex-between stat-row"
            style={{ marginBottom: "10px", animationDelay: "100ms" }}
          >
            <span style={{ fontSize: "12px", color: "var(--text-secondary)" }}>
              Échoué
            </span>
            <span
              className="badge badge-error stat-value"
              style={{ margin: 0 }}
            >
              {animatedFailed}
            </span>
        ...
        {report.stats.missing > 0 ? (
          ...
          >
            ...
            <span
              className="badge badge-warning stat-value"
              style={{ margin: 0 }}
            >
              {animatedMissing}
            </span>
        ...
        {report.stats.skipped > 0 ? (
          ...
          >
            <span style={{ fontSize: "12px", color: "var(--text-secondary)" }}>
              ...
              <span
                className="info-tip"
                data-tooltip="Non modifiable via l'API Webflow (placeholder, libellé de bouton submit) — à définir manuellement dans le Designer"
              >
                &#9432;
              </span>
            </span>
            <span
              className="badge badge-neutral stat-value"
              style={{ margin: 0 }}
            >
              {animatedSkipped}
            </span>
      ...
      {isMultiPage && report.multiPageReports ? (
        <>
          ...
          >
            {report.multiPageReports.map(
              (pageReport: DeploymentReport, idx: number) => {
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: "4px" }}>
                        ...
                        {pageReport.stats.skipped > 0 ? (
                          <span
                            className="badge badge-neutral"
                            style={{ margin: 0, fontSize: "10px" }}
                          >
                            {pageReport.stats.skipped}
                          </span>
                        ) : null}
                      </div>
                    </summary>
      ...
      <button onClick={onClose} className="btn-link mt-4">
        Fermer
      </button>
    </div>
  );
